							...
							enum modem_cellular_event evt)
{
	...
	switch (evt) {
	case MODEM_CELLULAR_EVENT_TIMEOUT:
		if (IS_ENABLED(CONFIG_MODEM_CELLULAR_USE_POWER_DOWN_SCRIPT) && (config->power_down_script != NULL)) {
...
static int modem_cellular_on_state_enter(struct modem_cellular_data *data)
{
	...
	LOG_DBG("ENTER STATE %s", modem_cellular_state_str(data->state));
	switch (data->state) {
	case MODEM_CELLULAR_STATE_IDLE:
...
static int modem_cellular_on_state_leave(struct modem_cellular_data *data)
{
	...
	LOG_DBG("LEAVE STATE %s", modem_cellular_state_str(data->state));
	switch (data->state) {
	case MODEM_CELLULAR_STATE_IDLE:
